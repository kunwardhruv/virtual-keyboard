...
# ✅ Variable to prevent continuous typing
pressed = False
...
while True:
    ...
    if hands:
        ...
        for button in buttonList:
            ...
            # check finger tip inside button
            if x < lmList[8][0] < x + w and y < lmList[8][1] < y + h:
                cv2.rectangle(img, (x-5, y-5), (x + w+5, y + h+5), (175, 0, 175), cv2.FILLED)
                cv2.putText(img, button.text, (x + 20, y + 65),
                            cv2.FONT_HERSHEY_PLAIN, 4, (255, 255, 255), 4)

                # distance between index (8) and middle (12) finger
                p1 = lmList[8][:2]
                p2 = lmList[12][:2]
                l, info, img = detector.findDistance(p1, p2, img)

                if l < 30 and not pressed:
        ...
        # ✅ Reset press when fingers apart
        if pressed:
